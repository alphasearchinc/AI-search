...
| `python-embedder/app` | Flask app package (routes/services) for generating embeddings. |
| `python-embedder/wsgi.py` | Entrypoint for running the embedder locally or in Docker. |
...
- **Python embedder**: `cd python-embedder && pip install -r requirements.txt && python wsgi.py` (or use Dockerfile).
